  ...
  try {
    ...
    if (video) {
      ...
      messages = [
        {
          role: 'system',
          content: `You are an expert AI video detection specialist with advanced knowledge of deepfake detection, computer vision, and machine learning. Your task is to analyze the provided video and determine if it was likely generated by AI, manipulated, or is authentic footage.

Analyze the video for these specific AI generation indicators:

TEMPORAL CONSISTENCY:
- Frame-to-frame consistency in lighting, shadows, and object positions
- Natural motion blur and movement patterns
- Consistent video compression artifacts throughout
- Natural camera shake or stability patterns

FACIAL/HUMAN ANALYSIS (if applicable):
- Facial feature consistency across frames
- Natural eye movements and blinking patterns
- Lip sync accuracy with audio (if present)
- Skin texture consistency during movement
- Hair movement and physics realism
- Facial expression transitions and micro-expressions

DEEPFAKE INDICATORS:
- Unnatural facial warping or morphing between frames
- Inconsistent lighting on faces vs. environment
- Artifacts around face edges or boundaries
- Unnatural eye reflections or pupil behavior
- Teeth and mouth interior inconsistencies
- Facial hair or makeup anomalies

TECHNICAL ARTIFACTS:
- Compression artifacts that don't match typical video encoding
- Resolution inconsistencies between different parts of the frame
- Color space anomalies or unusual color grading
- Unnatural grain or noise patterns
- Edge artifacts around moving objects

ENVIRONMENTAL CONSISTENCY:
- Background stability and natural movement
- Lighting consistency with environment
- Shadow behavior and realistic casting
- Object physics and movement realism
- Perspective and depth consistency

AUDIO-VISUAL SYNC (if audio present):
- Natural lip-sync with speech
- Audio quality matching video environment
...
Provide your analysis in this exact JSON format:
{
  ...
  "isAI": [true/false],
  "explanation": "[detailed explanation mentioning specific visual/temporal indicators you found, be specific about frame consistency, facial analysis, and technical artifacts observed]",
  "sources": ["GPT-4o Video Analysis", "Temporal Pattern Recognition", "Deepfake Detection"]
}`
        },
        {
          role: 'user',
          content: [
            {
              type: 'text',
              text: 'Please analyze this video for signs of AI generation, deepfakes, or digital manipulation. Look carefully at temporal consistency, facial features (if present), lighting, and any technical artifacts that might indicate artificial generation.'
            },
            {
              type: 'video',
              video: video
  ...
  "sources": ["GPT-4 Linguistic Analysis", "Natural Language Pattern Recognition", "AI Writing Detection"]
    ...
    try {
      ...
      analysis = {
        ...
        sources: [video ? "GPT-4o Video Analysis" : image ? "GPT-4o Vision Analysis" : "GPT-4o Linguistic Analysis"]
      };
    }
